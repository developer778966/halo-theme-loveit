...
(() => {
    var e = (() => {
        ...
        // 魔改 精确匹配 "package" 关键字后直到分号前的包路径部分，不包含 "package" 关键字、前后空白字符和分号
        var packagePathRule = {
            begin: /(?<=\bpackage\s+)\S+(?=\s*;)/,
            end: '', // 由于我们利用了正向前瞻在非空白字符后结束，这里不需要指定结束
            className: 'package-name', // 为匹配到的包路径部分分配类名 "package-name"
            returnEnd: false, // 保持默认行为，不改变匹配结束位置
            contains: [] // 包路径部分没有需要额外处理的子元素
        };
        // 魔改 精确匹配 "import" 关键字后直到分号前的包路径部分，不包含 "import" 关键字、前后空白字符和分号
        var importPathRule = {
            begin: /(?<=\bimport\s+)\S+(?=\s*;)/,
            end: '', // 由于我们利用了正向前瞻在非空白字符后结束，这里不需要指定结束
            className: 'import-name', // 为匹配到的包路径部分分配类名 "package-name"
            returnEnd: false, // 保持默认行为，不改变匹配结束位置
            contains: [] // 包路径部分没有需要额外处理的子元素
        };
        ...
        return e => {
            ...
            return {
                ...
                illegal: /<\/|#/,
                contains: [
                    // 魔改 添加新规则punctuationRules、stringRule、packagePathRule和importPathRule到 contains 数组中
                    punctuationRules,
                    stringRule,
                    packagePathRule,
                    importPathRule,
                ...
                {
                    // 注意这里的正则表达式需要精确匹配方法调用的模式，这里是一个示例
                    // 假设方法调用前可能有对象名或类型，后面跟着点号和方法名，最后是左圆括号
                    match: [
                        t, // 对象名或类型
                        /\s*/,
                        /\./,
                    ...
                    className: {
                        1: "typeName", // 对象名或类型的样式
                        5: "function", // 方法名的样式
                        7: "punctuation" // 左圆括号的样式
                    }
                }, {
                    match: /non-sealed/,
                    scope: "keyword"
