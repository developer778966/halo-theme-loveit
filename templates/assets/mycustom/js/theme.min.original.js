...
class Theme {
	constructor() {
		this.config = window.config, this.data = this.config.data, this.isDark = document.body.getAttribute("theme") === "dark", this.util = new Util, this.newScrollTop = this.util.getScrollTop(), this.oldScrollTop = this.newScrollTop, this.scrollEventSet = new Set, this.resizeEventSet = new Set, this.switchThemeEventSet = new Set, this.clickMaskEventSet = new Set, window.objectFitImages && objectFitImages()
	}
	initSVGIcon() {
...
`)
	...
	initMapbox() {
		this.config.mapbox && (mapboxgl.accessToken = this.config.mapbox.accessToken, mapboxgl.setRTLTextPlugin(this.config.mapbox.RTLTextPlugin), this._mapboxArr = this._mapboxArr || [], this.util.forEach(document.getElementsByClassName("mapbox"), e => {
			const {
				lng: n,
				lat: s,
				zoom: o,
				...
				scale: d,
				fullscreen: u
			} = this.data[e.id], t = new mapboxgl.Map({
				container: e,
				center: [n, s],
				zoom: o,
				minZoom: .2,
				style: this.isDark ? a : i,
				attributionControl: !1
			});
			r && (new mapboxgl.Marker)
				.setLngLat([n, s])
				.addTo(t), c && t.addControl(new mapboxgl.NavigationControl, "bottom-right"), l && t.addControl(new mapboxgl.GeolocateControl({
					positionOptions: {
						enableHighAccuracy: !0
					},
					showUserLocation: !0,
					trackUserLocation: !0
				}), "bottom-right"), d && t.addControl(new mapboxgl.ScaleControl), u && t.addControl(new mapboxgl.FullscreenControl), t.addControl(new MapboxLanguage), this._mapboxArr.push(t)
		}), this._mapboxOnSwitchTheme = this._mapboxOnSwitchTheme || (() => {
			this.util.forEach(this._mapboxArr, e => {
				const t = e.getContainer(),
					{
						lightStyle: n,
						darkStyle: s
					} = this.data[t.id];
				e.setStyle(this.isDark ? s : n), e.addControl(new MapboxLanguage)
			})
	...
	initComment() {
		if (this.config.comment) {
			if (this.config.comment.gitalk) {
				...
				const e = new Gitalk(this.config.comment.gitalk);
				e.render("gitalk")
			}
			if (this.config.comment.vssue) {
				const e = this.config.comment.vssue;
				new Vue({
					el: "#vssue",
					render: t => t("Vssue", {
						props: {
			...
			if (this.config.comment.valine && new Valine(this.config.comment.valine), this.config.comment.utterances) {
				const e = this.config.comment.utterances,
					t = document.createElement("script");
				t.src = "https://utteranc.es/client.js", t.setAttribute("repo", e.repo), t.setAttribute("issue-term", e.issueTerm), e.label && t.setAttribute("label", e.label), t.setAttribute("theme", this.isDark ? e.darkTheme : e.lightTheme), t.crossOrigin = "anonymous", t.async = !0, document.getElementById("utterances")
					.appendChild(t), this._utterancesOnSwitchTheme = this._utterancesOnSwitchTheme || (() => {
						const t = {
								type: "set-theme",
								theme: this.isDark ? e.darkTheme : e.lightTheme
							},
							n = document.querySelector(".utterances-frame");
						n.contentWindow.postMessage(t, "https://utteranc.es")
	...
	init() {
		try {
			this.initSVGIcon(), this.initTwemoji(), this.initMenuMobile(), this.initSwitchTheme(), this.initSearch(), this.initDetails(), this.initLightGallery(), this.initHighlight(), this.initTable(), this.initHeaderLink(), this.initMath(), this.initMermaid(), this.initEcharts(), this.initTypeit(), this.initMapbox(), this.initCookieconsent()
		} catch (e) {
			console.error(e)
		}
		window.setTimeout(() => {
			this.initToc(), this.initComment(), this.onScroll(), this.onResize(), this.onClickMask()
		}, 100)
	}
}
const themeInit = () => {
